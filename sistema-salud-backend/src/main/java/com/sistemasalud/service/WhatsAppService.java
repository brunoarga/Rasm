...
public class WhatsAppService {
    ...
    public void enviarPlantillaConEnlace(String telefono, String templateName, List<String> cuerpoParams, String link) {
        if (!enabled || token == null || token.isBlank() || phoneNumberId == null || phoneNumberId.isBlank()) {
            log.info("WhatsApp no configurado (enabled={}, token={}, phoneId={}); se omite plantilla {}",
                    enabled, token != null, phoneNumberId != null, templateName);
            return;
        ...
        try {
            ...
            if (link != null && !link.isBlank()) {
                components.add(Map.of(
                        ...
                        "sub_type", "url",
                        "index", "0",
                        "parameters", List.of(Map.of("type", "text", "text", link))));
            }
            ...
            restClient.post()
                    ...
                    .retrieve()
                    .toBodilessEntity();
            log.info("WhatsApp plantilla '{}' (con enlace) enviada a {}", templateName, e164);
        } catch (Exception e) {
            log.error("Error al enviar WhatsApp a {}: {}", telefono, e.getMessage());
        }
    }
