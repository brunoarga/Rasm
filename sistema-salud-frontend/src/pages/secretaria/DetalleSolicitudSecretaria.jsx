...
export default function DetalleSolicitudSecretaria() {
  ...
  const handleDerivarCentro = async (idCentro) => {
    const yaSeleccionado = parseInt(centroSel) === idCentro && sol.idCentroSalud != null;
    if (yaSeleccionado) {
  ...
  const handleAsignarTurno = async (horaSlot) => {
    try {
      ...
      toast.error(err.response?.data?.mensaje || 'Error al asignar turno');
    }
  };
        ...
        <div className={`bg-white border rounded-xl p-6 ${sol.prioridad === 'URGENTE' ? 'border-red-200' : 'border-slate-200'}`}>
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-600">{sol.estado}</span>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            {step === 'info' && (
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                <h3 className="text-sm font-bold text-slate-700 mb-4">Paso 1: Derivar a Centro de Salud</h3>
            ...
            {step === 'profesional' && (
              <div className="bg-white border border-slate-200 rounded-xl p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-bold text-slate-700">Paso 2: Seleccionar Profesional</h3>
                  {sol.nombreCentroSalud && !cambiandoCentro && (
                ...
                {cambiandoCentro && (
                  <div className="mb-4">
                    ...
                    {centros.length === 0 ? (
                      <p className="text-xs text-slate-400">No hay centros disponibles. <button onClick={handleBuscarCentros} className="text-blue-600 underline">Intentar de nuevo</button></p>
                    ) : (
